...
int main(int argc, char *argv[]){
  ...
  char img_path[100] = IMG_DIR;
  if(argc > 1){
    strcat(img_path, argv[1]);
  }else{
    strcat(img_path, "8.bmp");
  }

  read_mnist_bmp(img_path, &img, &img_info);
  printf("Size  : %d px\n", img_info->size);
  printf("Width : %d px\n", img_info->width);
  printf("Height: %d px\n", img_info->height);
  ...
  /*Reference imp*/
  /*CPU implement like using stack register 
  /* for(int och=0;och<out_ch;och++){ */
  /*   for(int i=0;i<out_h;i++){ */
  /*     for(int j=0;j<out_w/4;j++){ */
  /* 	float tmp[4]={}; */
  /* 	for(int ki=0;ki<kh;ki++){ */
  /* 	  for(int kj=0;kj<kw;kj++){ */
  /* 	    int widx  = och*kh*kw+ki*kw+kj; */
  /* 	    for(int mj=0;mj<4;mj++){ */
  /* 	      int inidx = (i-pady+ki)*in_w+((j*4+mj)-padx+kj); */
  /* 	      tmp[mj]+=input_array[inidx]*W1[widx]; */
  /* 	    } */
  /* 	  } */
  /* 	} */
  /* 	for(int mj=0;mj<4;mj++){ */
  /* 	  tmp[mj]+=b1[och]; //add bias */
  /* 	  tmp[mj] = fmax(tmp[mj], 0); // relu */
  /* 	  dst1_array[och][i*out_w+(j*4+mj)]=tmp[mj]; */
  /* 	} */
  /*     } */
  /*   } */
  /* } */
    ...
    for(int i=0;i<out_h;i++){
      for(int j=0;j<out_w/4;j++){
        ...
        for(int ki=0;ki<kh;ki++){
          for(int kj=0;kj<kw;kj++){
            int widx  = och*kh*kw+ki*kw+kj;
            int inidx = (i-pady+ki)*in_w+((j*4)-padx+kj);
            wset(W1[widx]); //set Weight in Register
            fma_4elm(input_array[inidx],input_array[inidx+1],
              input_array[inidx+2],input_array[inidx+3]);
          }
        }
        fadd_4elm(b1[och]);
        for(int mj=3;mj>=0;mj--){
          float tmp=fpop();
          tmp = fmax(tmp, 0); // relu
          dst1_array[och][i*out_w+(j*4+mj)]=tmp;
        }
      }
    }
  }
  ...
  for(int i=0;i<math;i++){
    ...
    for(int j=0;j<matw;j++){
      fma_r(W2[j],dst2_array[j]);
    }
    fadd_r(b2[i]);  //add bias
    tmp = fpop();
    dst3_array[i] = fmax(tmp, 0); // relu
  ...
  for(int i=0;i<math;i++){
    ...
    for(int j=0;j<matw;j++){
      ...
      fma_r(W3[j*math+i],dst3_array[j]);
    }
    fadd_r(b3[i]);
    dst4_array[i]=fpop();
    
    //dst4_array[i] = tmp + b3[i];
  }
